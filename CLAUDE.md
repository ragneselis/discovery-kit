# so-what - Contributor & Maintainer Guide

This file is for people developing the so-what plugin. **It is not loaded at
...
so-what teaches product builders product management by forcing structured
...
- `skills/so-what/` - root hub. Shows progress across artifacts, recommends
...
- Interview notes/transcripts are an any-time input in `product/interviews/` (one
  ...
  `so-what: synthesized`, then stamps each handled file with that marker. The hub
...
- `skills/feature/` - a parallel mode for existing products. Once `product/PRODUCT.md`
  exists, a new feature goes through `so-what:feature` (scoped discovery from the
...
- **Invocation / detection** - the `description` frontmatter of
  `skills/so-what/SKILL.md`. That frontmatter is the only thing in the system
...
- **Open-questions register protocol** - a second inlined block (`## Open
  ...
  `skills/so-what/SKILL.md`. The register also carries a `revise -> <artifact>` item type: when new evidence contradicts an upstream artifact, a skill logs a revise item naming it, the hub surfaces it first and routes back to that skill, and the gate forces a decision. This is so-what's lightweight backward-propagation - no per-artifact status lines.
- **Implementation guard** (block building until `problem-statement.md` and
  `PRODUCT.md` exist) - `skills/so-what/SKILL.md`.
- **Build-agnostic handoff / Superpowers coexistence** - `skills/product-doc/SKILL.md`
  ...
  so-what skills never invoke Superpowers skills otherwise, and never touch
  implementation, debugging, code review, or testing.

## Conventions

- Each artifact opens with `# Title` and `> Generated by so-what - [date]`,
