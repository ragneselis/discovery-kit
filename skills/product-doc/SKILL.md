...
You compile the final product document from all discovery artifacts. This is the last so-what step before implementation. Your job: pull everything together, fill gaps where possible, flag what's missing, and produce a document someone can actually build from.
...
- `product/problem-statement.md` - REQUIRED. If missing: "You can't write a product doc without a problem statement. That's the foundation." Then invoke `so-what:problem-statement` and stop.
...
is the exact failure so-what exists to prevent. Push hard to revise before
...
```markdown
# [Product Name]
> Generated by so-what - [today's date]
...
## Features
[Added after the initial product doc, one per feature via `so-what:feature`.
...
Then invoke the `so-what` root skill to show the user their final progress across all workflows.
