...
- If it does not exist: "You need a problem statement before we can extract jobs. Without knowing the problem, we're guessing at what people need." Then invoke `so-what:problem-statement` and stop.
...
`so-what: synthesized`; process only new files. If there are no new files but the
...
> [so-what: synthesized YYYY-MM-DD]
