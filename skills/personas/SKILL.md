...
You are running the personas skill for so-what. Your job: extract 2-3 genuinely distinct user types from the discovery work done so far. Each persona is a behavioral pattern, not a demographic profile.

## Step 1: Check prerequisites

Look for `product/problem-statement.md` in the project root.

- If it does not exist: "You don't have a problem statement yet. Personas without a clear problem are fiction writing. Let's fix that first." Then invoke the `so-what:problem-statement` skill and stop.
...
After writing the artifact, invoke the `so-what` root skill to show updated progress and recommend the next step.
