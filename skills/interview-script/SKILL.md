...
Then invoke the `so-what:problem-statement` skill and stop.
...
```markdown
# Interviews
Put one interview per file here - notes or transcript, any format. so-what picks
...
Then invoke the `so-what` root skill to show updated progress.
